...
function extendMaterial(
  ...
  const uniforms = THREE.UniformsUtils.clone(baseUniforms) as Record<
    string,
    { value: unknown }
  >
  ...
  if ('envMapIntensity' in defaults)
    uniforms.envMapIntensity.value = (defaults as THREE.MeshStandardMaterial).envMapIntensity
...
const CanvasWrapper = ({
  ...
  <Canvas dpr={dpr} frameloop="always" className="beams-container" gl={{ powerPreference: 'high-performance' }}>
    {children}
  </Canvas>
)
...
const MergedPlanes = forwardRef<
  THREE.Mesh,
  MergedPlanesProps
>(function MergedPlanes({ material, width, count, height, heightSegments = 100 }, ref) {
  ...
  useFrame((_, delta) => {
    ...
    if (mat?.uniforms?.time?.value !== undefined) {
...
type PlaneNoiseProps = MergedPlanesProps & { material: THREE.ShaderMaterial }

const PlaneNoise = forwardRef<THREE.Mesh, PlaneNoiseProps>(function PlaneNoise(
  props,
  ref
) {
  return (
...
function DirLight({
  ...
  useEffect(() => {
    ...
    const cam = (dir.current as THREE.DirectionalLight & { shadow: { camera: THREE.OrthographicCamera } }).shadow?.camera
    if (!cam) return
    cam.top = 24
    cam.bottom = -24
    cam.left = -24
    cam.right = 24
    cam.far = 64
    ;(dir.current as THREE.DirectionalLight & { shadow: { bias: number } }).shadow.bias = -0.004
  }, [])
  return (
    <directionalLight
      ref={dir}
